...
import { InfluencerHistoryDialog } from "./influencer-history-dialog"; // Importar o novo componente
...
export function DayDetailsDialog({ date, posts, influencers, products, open, onOpenChange, onEditPost }: DayDetailsDialogProps) {
  const [historyInfluencer, setHistoryInfluencer] = React.useState<Influencer | null>(null);
  ...
  const handleViewHistory = (influencerId: string) => {
    const influencer = getInfluencer(influencerId);
    if (influencer) {
      setHistoryInfluencer(influencer);
    }
  };
    ...
    <>
                  ...
                  <div className="flex justify-between items-start">
                      <div>
                          ...
                          <div className="flex flex-wrap gap-2 items-center text-sm text-muted-foreground mb-3">
                              <Badge variant="outline" className="cursor-pointer hover:bg-secondary" onClick={() => handleViewHistory(post.influencerId)}>
                                {getInfluencer(post.influencerId)?.name || 'N/A'}
                                <History className="h-3 w-3 ml-1.5" />
                              </Badge>
      ...
      {historyInfluencer && (
        <InfluencerHistoryDialog
          ...
          open={!!historyInfluencer}
          onOpenChange={(isOpen) => !isOpen && setHistoryInfluencer(null)}
        />
      )}
    </>
  );
